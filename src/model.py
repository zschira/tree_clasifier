...
class LeafNet():
    ...
    def build_model(self):
        ...
        rgb_down = layers.Conv2D(12, 5, activation="relu")(rgb_down)
        rgb_down = layers.BatchNormalization()(rgb_down)
        rgb_down = layers.MaxPool2D(2)(rgb_down)
        rgb_down = layers.Dropout(0.2)(rgb_down)
        rgb_down = layers.Conv2D(12, 5, activation="relu")(rgb_down)
        rgb_down = layers.BatchNormalization()(rgb_down)
        rgb_down = layers.Conv2D(24, 5, activation="relu")(rgb_down)
        rgb_down = layers.BatchNormalization()(rgb_down)
        rgb_down = layers.MaxPool2D(2)(rgb_down)
        rgb_down = layers.Dropout(0.2)(rgb_down)
        rgb_down = layers.Conv2D(30, 5, activation="relu")(rgb_down)
        rgb_down = layers.BatchNormalization()(rgb_down)
        rgb_down = layers.MaxPool2D(2)(rgb_down)
        rgb_down = layers.Dropout(0.2)(rgb_down)
        """
        ...
        low_res = layers.Concatenate(axis=3)([low_res, rgb_down])
        low_res = layers.Conv2D(64, 2, activation="relu")(low_res)
        low_res = layers.BatchNormalization()(low_res)
        low_res = layers.Conv2D(64, 2, activation="relu")(low_res)
        low_res = layers.BatchNormalization()(low_res)
        ...
        # Las 3D network
        las_net = layers.Conv3D(1, 2, activation="relu")(las_input)
        las_net = layers.BatchNormalization()(las_net)
        las_net = layers.Conv3D(1, 2, activation="relu", padding="same")(las_input)
        las_net = layers.BatchNormalization()(las_net)
        las_net = layers.Dropout(0.2)(las_net)
        las_net = layers.MaxPool3D(2)(las_net)
        las_net = layers.Conv3D(32, 3, activation="relu")(las_net)
        las_net = layers.BatchNormalization()(las_net)
        las_net = layers.Conv3D(32, 3, activation="relu", padding="same")(las_net)
        las_net = layers.BatchNormalization()(las_net)
        las_net = layers.MaxPool3D(2)(las_net)
        las_net = layers.Conv3D(64, 3, activation="relu")(las_net)
        las_net = layers.BatchNormalization()(las_net)
        las_net = layers.Conv3D(64, 3, activation="relu", padding="same")(las_net)
        las_net = layers.BatchNormalization()(las_net)
        las_net = layers.MaxPool3D(2)(las_net)
        las_net = layers.Conv3D(128, 2, activation="relu")(las_net)
        las_net = layers.BatchNormalization()(las_net)
        las_net = layers.Conv3D(128, 2, activation="relu", padding="same")(las_net)
        las_net = layers.BatchNormalization()(las_net)
        ...
        fully_con = layers.concatenate([low_res, las_net])
        fully_con = layers.Dense(1024)(fully_con)
        fully_con = layers.Dropout(0.5)(fully_con)
        fully_con = layers.Dense(1024)(fully_con)
        fully_con = layers.Dropout(0.2)(fully_con)
